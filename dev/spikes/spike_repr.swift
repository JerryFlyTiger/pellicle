...
let N = 1_000_000, SUMS = 20, LOOP = 20_000_000

// --- Rep A: indirect enum (each cons is an ARC box) ---
indirect enum VA { case nilv; case int(Int); case cons(VA, VA) }
@inline(never) func benchA(naive: Bool) -> (Double, Int) {
    let t0 = now()
    var list = VA.nilv
    for i in 0..<N { list = .cons(.int(i), list) }
    var total = 0
    for _ in 0..<SUMS { var p = list; while case let .cons(car, cdr) = p { if case let .int(n) = car { total &+= n }; p = cdr } }
    var acc = VA.int(0)
    for i in 0..<LOOP { if case let .int(a) = acc { acc = .int(a &+ (i & 7)) } }
    if case let .int(a) = acc { total &+= a }
    let t1 = now() - t0
    if !naive { while case let .cons(_, cdr) = list { list = cdr } }   // iterative teardown
    return (t1, total)
}

// --- Rep B: enum with final-class cons payload (ARC) ---
final class ConsB { var car: VB; var cdr: VB; init(_ a: VB, _ d: VB) { car = a; cdr = d } }
enum VB { case nilv; case int(Int); case cons(ConsB) }
@inline(never) func benchB(naive: Bool) -> (Double, Int) {
    let t0 = now()
    var list = VB.nilv
    for i in 0..<N { list = .cons(ConsB(.int(i), list)) }
    var total = 0
    for _ in 0..<SUMS { var p = list; while case let .cons(c) = p { if case let .int(n) = c.car { total &+= n }; p = c.cdr } }
    var acc = VB.int(0)
    for i in 0..<LOOP { if case let .int(a) = acc { acc = .int(a &+ (i & 7)) } }
    if case let .int(a) = acc { total &+= a }
    let t1 = now() - t0
    if !naive { while case let .cons(c) = list { list = c.cdr; c.cdr = .nilv } }
    return (t1, total)
}

// --- Rep C: tagged 64-bit word + manual arena (no ARC; a GC would be separate) ---
struct VC { var raw: UInt64
    @inline(__always) static func int(_ n: Int) -> VC { VC(raw: UInt64(bitPattern: Int64(n) << 1) | 1) }
...
struct CellC { var car: VC; var cdr: VC }
final class ArenaC {
    var cells: UnsafeMutablePointer<CellC>; var count = 1
    init(cap: Int) { cells = .allocate(capacity: cap) }
    deinit { cells.deallocate() }
    @inline(__always) func cons(_ a: VC, _ d: VC) -> VC { let i = count; count += 1; cells[i] = CellC(car: a, cdr: d); return VC(raw: UInt64(i) << 1) }
...
@inline(never) func benchC() -> (Double, Int) {
    ...
    var total = 0
    for _ in 0..<SUMS { var p = list; while p.raw != 0 { let a = arena.car(p); if a.isInt { total &+= a.intValue }; p = arena.cdr(p) } }
    var acc = VC.int(0)
...
for run in 0..<3 {
    let r: (Double, Int)
    switch rep { case "A": r = benchA(naive: naive); case "B": r = benchB(naive: naive); default: r = benchC() }
    print(String(format: "rep %@%@ run %d: %.3fs (checksum %d)", rep, naive ? "/naive" : "", run, r.0, r.1))
}
