...
@_silgen_name("sys_icache_invalidate") func sys_icache_invalidate(
    _ start: UnsafeMutableRawPointer, _ len: Int)
...
guard
    let mem = mmap(
        ...
        MAP_PRIVATE | MAP_ANON | MAP_JIT, -1, 0), mem != MAP_FAILED
else {
    print("mmap MAP_JIT failed errno=\(errno)")
    exit(1)
}
