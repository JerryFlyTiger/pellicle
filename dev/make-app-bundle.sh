#!/bin/sh
# Assemble .build/pellicle.app, a minimal, hardened-runtime, ad-hoc-signed macOS app
# bundle around the release binary, so:
#
#   - the Dock shows a real app icon instead of the generic terminal icon a bare
#     executable gets (assets/icon/pellicle.icon, compiled by the icon step below);
...
#     `.build/release/pellicle` has no entitlements at all, so MAP_JIT and the dlopen
...
# the bundle fresh each run (any previous .build/pellicle.app is removed first), so a
...
# (dev/pellicle.entitlements) carry two keys, both required by the 2026-09-05 spike
...
echo "==> swift build -c release --product pellicle"
swift build -c release --product pellicle

echo "==> swift build -c release --product SelfTestProbe"
swift build -c release --product SelfTestProbe

BIN="$ROOT/.build/release/pellicle"
...
TMP_APP="$ROOT/.build/pellicle.app.tmp.$$"
ACTOOL_TMP=""
cleanup() {
    rm -rf "$TMP_APP"
...
<dict>
    ...
    <string>pellicle</string>
    <key>CFBundleExecutable</key>
    <string>pellicle</string>
    <key>CFBundleIdentifier</key>
    <string>app.pellicle</string>
...
# Icon: assets/icon/pellicle.icon, generated by dev/gen-icon.py; the actool/.icon step
# is Reticle's (dev/make-app-bundle.sh, read as prior art). The step stays conditional on
# the artwork existing so a tree without it still produces a signed, launchable bundle.
#
# actool constraint, quoted from its own output on 2026-09-07: "Linear gradients require
# exactly 2 colors" -- icon.json's three-stop first draft made actool throw
# (-[__NSPlaceholderArray initWithObjects:count:] nil object) rather than report the
# error cleanly, so a future edit to the gradient is limited to two stops.
ICONPKG="$ROOT/assets/icon/pellicle.icon"
if [ -d "$ICONPKG" ]; then
    if ACTOOL=$(xcrun --find actool 2>/dev/null); then
        ...
        if "$ACTOOL" --output-format human-readable-text --notices --warnings \
                ...
                --app-icon pellicle --output-partial-info-plist "$ACTOOL_TMP/partial.plist" \
            ...
            && plutil -insert CFBundleIconName -string pellicle "$TMP_APP/Contents/Info.plist"
        then
            echo "compiled assets/icon/pellicle.icon -> $RES_DIR/Assets.car (actool found at $ACTOOL)"
        else
    ...
    echo "no assets/icon/pellicle.icon: bundle carries no icon (run dev/gen-icon.py)"
fi

echo "==> signing $FRAMEWORKS_DIR/libSelfTestProbe.dylib (ad-hoc, hardened runtime)"
codesign --force --sign - --options runtime \
    --entitlements "$ROOT/dev/pellicle.entitlements" --timestamp=none \
