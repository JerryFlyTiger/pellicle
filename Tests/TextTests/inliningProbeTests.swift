...
@Suite("Text inlining probe")
struct TextInliningProbeTests {
    @Test("hot and cold probes agree on every input")
    func hotAndColdAgree() {
        let hot = TextHotProbe()
        let cold = TextColdProbe()
    ...
    func seedMatters() {
        #expect(
            TextHotProbe(seed: 31).packedByteLength(7) != TextHotProbe(seed: 32).packedByteLength(7)
        )
        #expect(
            TextColdProbe(seed: 31).packedByteLength(7)
                != TextColdProbe(seed: 32).packedByteLength(7))
    }
