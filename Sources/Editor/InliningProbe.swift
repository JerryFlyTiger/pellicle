...
///
/// `editorHotProbe` and `editorWarmProbe` must compile to code containing no reference to
/// the Text/Lisp hot/warm symbols; `editorColdProbe` must still call the Text/Lisp cold
/// symbols.
import Lisp
import Text
...
package func editorWarmProbe(_ n: UInt64) -> UInt64 {
    let text = TextWarmProbe()
    let lisp = LispWarmProbe()
    var acc: UInt64 = 0
    var i: UInt64 = 0
    while i < n {
        acc = acc &+ text.packedByteLength(i) &+ lisp.packedByteLength(i)
